...
const Sidebar = () => {
  ...
  const { selectedProperty, setSelectedProperty } = useStore();
      ...
      <div className="p-6 border-b border-gray-100 bg-gray-50">
        {activeUnit ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-gray-900">Unit Details</h1>
          </div>
        ) : (
        ...
        {activeUnit ? (
          ...
          <div className="p-6 flex flex-col gap-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className={`p-5 rounded-xl border ${
              ...
              <div className="mt-4 pt-4 border-t border-gray-200/50 flex justify-between items-end">
                 <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">{activeUnit.price}</p>
                 </div>
                 <div className="text-right">
                    <p className="text-xs text-gray-500 uppercase font-semibold">Area</p>
                    <p className="text-lg font-bold text-gray-700">{activeUnit.sqft} <span className="text-sm font-normal">sqft</span></p>
                 </div>
              </div>
            </div>
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                        ...
                        <p className="font-semibold text-gray-800">{activeUnit.bathrooms}</p>
                    </div>
                    {activeUnit.features.map((feature, index) => (
                        <div key={index} className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                            <p className="text-xs text-gray-400 mb-1">Feature</p>
                            <p className="font-semibold text-gray-800 text-xs">{feature}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="mt-auto pt-6">
                ...
                >
                    {activeUnit.isAvailable ? "Book Viewing" : "Unavailable"}
                </button>
            </div>
          </div>
